...
const CartDrawer = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    await placeOrder(user, {
      address: orderType === "delivery" ? `${platform} - ${address}` : "Pickup",
      time: preferredTime,
    });
  };
    ...
    <div className="min-h-screen pt-28 bg-[#0a0a0a] text-white flex flex-col items-center px-4">
      ...
      {cartItems.length === 0 ? (
        ...
        <div className="w-full max-w-3xl flex flex-col gap-6 mb-20">
          ...
          {/* CART ITEMS */}
          {cartItems.map((item) => (
            <div key={item._id} className="bg-[#141414] border border-gray-800 rounded-xl shadow p-4 flex items-center justify-between">
