    ...
    <div className="flex flex-col items-center" style={{ width: 88 }}>
      ...
      <div className="relative rounded-2xl overflow-hidden" style={{ height: containerH, width: "100%" }}>
        ...
        <div className="absolute inset-x-0 z-10 pointer-events-none mx-2 rounded-xl"
          style={{ top: paddingV, height: ITEM_H, background: "rgba(245,194,122,0.07)", border: "1.5px solid rgba(245,194,122,0.3)" }} />
        <div className="absolute inset-x-0 bottom-0 z-10 pointer-events-none"
          style={{ height: paddingV, background: "linear-gradient(to top, #161616 0%, transparent 100%)" }} />
        <div ref={listRef} onScroll={handleScroll} className="absolute inset-0 overflow-y-scroll"
          style={{ scrollbarWidth: "none", paddingTop: paddingV, paddingBottom: paddingV }}>
          {items.map((item, i) => {
            const isSel = item === selected;
            return (
              <div key={item} onClick={() => { onSelect(item); scrollToIndex(i, "smooth"); }}
                className="flex items-center justify-center cursor-pointer" style={{ height: ITEM_H }}>
                <span className="font-mono font-black transition-all duration-200 select-none"
                  style={{ fontSize: isSel ? 26 : 16, color: isSel ? "#f5c27a" : "#3f3f46" }}>
